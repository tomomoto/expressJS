function getFollowedByAjax(followersOwner){
    var limit = 5;
    ...
    httpRequest.onreadystatechange = function() {
        if (httpRequest.readyState == 4 && httpRequest.status == 200) {
            if(httpRequest.response.length != 2) {
                ...
                attachFollowed(parsedData);
                getFollowedByAjaxClicks += 1;
            }
        }
    };
    httpRequest.open("GET", "/getAjaxUserFollowed?offset="+limit*getFollowedByAjaxClicks+"&owner="+followersOwner+"&limit="+limit,true);
    httpRequest.send();
}

function attachFollowed(parsedData){
    for (var i=0;i<parsedData.length;i++){
        ...
        /*+
         ...
         '</div>'+
         '</div>';*/
        document.getElementById('followedModalBody').appendChild(followerP);
